...
- 2026-09-10 — Icon resolution moved to the backend (`pkg/icons`) with rule-based fuzzy matching — exact file-name lookup broke on any process with a space in its name (`Docker Desktop.exe`) and on macOS names (`Google Chrome`). Scores: 100 identical, 95 separators ignored, 85 platform / glue noise ignored, 50–80 token containment, 40 glued prefix; `MinScore` 40. Bundled icons are listed from the embedded `frontend/dist/app-icons` via `fs.Sub`; discovery now returns `DiscoveredAppView{Name, ChildCount, IconSrc}` and `ResolveIcon(name)` replaced `GetPrivateIcons`. Private icons beat bundled on equal score. Icon design rules live in `assets/APP-ICON-SPEC.md`.
